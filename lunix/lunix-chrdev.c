...
static int lunix_chrdev_state_update(struct lunix_chrdev_state_struct *state)
{
	...
	if (updated) {
	    ...
	    switch (state->mode) {
			...
			case COOKED:
				unsigned char sign = (__data >= 0) ? '+' : '-';
				long dec = __data / 1000;
				long frac = __data % 1000;
...
static ssize_t lunix_chrdev_read(struct file *filp, char __user *usrbuf, size_t cnt, loff_t *f_pos)
{
	...
	/* Lock? */
    if (!mutex_is_locked(&state->lock) && mutex_lock_interruptible(&sensor->lock)) {
    ...
    rfsize = (*fpos + cnt >= state->buf_lim) ? state->buf_lim - *fpos : cnt;
